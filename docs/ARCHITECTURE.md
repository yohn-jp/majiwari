...
**#45 decision (blocks #30).** Inspected against current `yohn-jp/mottainai` `main` source and exercised directly (not inferred from names): `mottainai task status`/`mottainai_workflow_task_status` are scoped to exactly one `--workspace` (no enumeration, no `--task-id`); Manager's `GET /api/v1/manager/sessions` does enumerate but is bound to one `--workspace` per running `mottainai manager` process (a second `ManagerSessionService` instance bound to a different repo sees zero of another repo's sessions against the same store, verified against real fixture code), is loopback-only, unversioned, self-described by its own README as "non-authoritative UI projection," and its projection includes the absolute `worktreePath`; no exported library contract exists (no `exports`/`main`, no workspace packages). No current supported Mottainai interface satisfies #45's bounded/cross-repository/public-contract requirements, so #30's dependency is the smallest upstream addition captured and linked at `yohn-jp/mottainai#539` (read-only `mottainai task list --json` cross-repo enumeration, `mottainai task status --task-id <id> --json` keyed resolve, and a matching `mottainai_workflow_task_list` MCP tool) rather than coupling this repository to Manager internals or Mottainai/Nawabari's private state store. Full evaluation recorded on #30.
...
## Mottainai's place in this platform (#56)

Mottainai (`yohn-jp/mottainai`) implements its own native harness-delegation MCP server -- `mottainai_delegate_work`, `mottainai_inspect_work`, `mottainai_continue_work`, `mottainai_cancel_work`, and the discovery tool `mottainai_harness_capabilities` -- entirely within its own codebase (`yohn-jp/mottainai#548`, `docs/mcp-harness-delegation.md` there). This repository does not implement, modify, or reinterpret any of that: `adapters/mottainai/` is a normal trusted adapter that launches Mottainai's own packaged, documented `mottainai-mcp` stdio entrypoint (published in that package's `bin` map, backed by `dist/mcp.js`, resolved from `PATH`) as a child process and registers it through the same generic `registry/`/`gateway/` contract every other adapter uses -- `adapters/mottainai/src/manifest.js` builds its manifest with `@majiwari/gateway`'s own `createStdioGatewayTransport`, exactly as OCR's and Inari's manifests do, so it can be published through `createRegistryGateway` at `/mcp/mottainai` with zero Mottainai-specific code in `gateway/` or `registry/`.

Unlike OCR and Inari, this adapter declares no manifest-level `health()` and no domain-specific compatibility check: it has no CLI JSON contract of its own to validate against, only an MCP stdio process to launch, so the registry's own lifecycle state (`registered`/`running`/`errored`) is already the correct, redaction-safe health signal. `listTools()` reads the live tool list from the same connected `mcpClient` the gateway bridges downstream sessions onto -- the identical pattern OCR/Inari use -- so tool names, input schemas, output shapes, and negotiated capability/version metadata reach `/mcp/mottainai` exactly as Mottainai's own process reports them, with nothing renamed, reshaped, cached, or reinterpreted in between. `adapters/mottainai/test/gateway-integration.test.js` proves this against a fixture stand-in for the real `mottainai-mcp` binary (the real package is not a dependency of this repository, matching Mottainai's own instruction that Majiwari must never import its internals): registration, start, publication at `/mcp/mottainai`, a delegation call followed by an inspection call resolving the exact opaque `workId` the launched process minted, and isolation from a sibling adapter published on the same gateway when Mottainai is stopped.

The trusted resident catalog (`runtime/src/catalog.js`) accepts a closed `mottainai` config entry (`runtime/src/config.js`): `enabled` plus one optional absolute `config` path, passed straight through as Mottainai's own documented `--config <path>` launch flag. No `repo`, `targets`, executable, argument array, or environment map is accepted for this id -- the entrypoint that runs is fixed to the packaged `mottainai-mcp` name, never selected by resident config, so an operator can choose whether this adapter runs but never what it executes. This `mottainai` catalog *id* is unrelated to OCR's existing nested `mottainai` config field (`open-code-review`'s target-discovery provider, #30/#539, `adapters/open-code-review/src/mottainai-target-provider.js`) -- the two live at different paths in the same config schema and serve different purposes: one selects a target-discovery CLI contract for OCR, the other registers Mottainai's own native MCP server as a sibling adapter.

Majiwari does not become required for local Mottainai MCP use -- `mottainai-mcp` remains directly usable without this gateway -- and does not acquire ownership of Mottainai/Nawabari task/worktree state; this adapter never caches, infers, or synthesizes any of that.
