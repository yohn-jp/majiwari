...
- ~~Migrating the OCR adapter onto `registry/`'s manifest/registry contract~~ -- done: `adapters/open-code-review/src/manifest.js` builds the adapter's manifest and `src/runtime.js` (`npm run start:registry`) registers and starts it through `AdapterRegistry`. `src/server.js` (`npm start`) is unchanged and keeps working standalone; the registry runtime is a second way to host the same server, not a replacement.
