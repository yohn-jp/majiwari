...
- `adapters/inari/src/core.js` also pins the exact `gh-inari` machine-readable contract (`EXPECTED_INARI_NAME`, `EXPECTED_INARI_PROTOCOL`, `MINIMUM_INARI_VERSION`) this adapter is written against, and `checkAdapterHealth()` -- shared by the `adapter_health` tool, `doctor.js`, and the manifest's own `health()` -- fails deterministically, with a `detail` naming exactly what is incompatible, if the installed `inari` does not match it. `.github/workflows/ci.yml`'s `inari-contract` job installs that exact pinned version and verifies the same machine-readable surface against the real CLI, rather than validating against whatever `gh-inari` version happens to be latest at CI run time.
