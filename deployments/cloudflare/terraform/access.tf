# The Worker's path to the gateway: a Workers VPC Service binding,
...
resource "cloudflare_zero_trust_access_policy" "mcp" {
  ...
  lifecycle {
    precondition {
      condition     = length(var.mcp_access_allowed_email_domains) > 0 || length(var.mcp_access_allowed_emails) > 0
      error_message = "At least one mcp_access_allowed_email_domains or mcp_access_allowed_emails value must be configured; an empty allowlist is not permitted."
    }
  }
}
