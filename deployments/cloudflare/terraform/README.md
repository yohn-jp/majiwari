...
token needs `Access: Apps Write`, `Access: Policies Write`, and
`Connectivity Directory Admin` (to create the VPC Service). Do not put it in
a `.tfvars` file or command-line argument.
...
Use encrypted or remote state and never commit state files. The local
`.gitignore` is a guard, not a replacement for protected state storage.

Copy the `/mcp` application's
